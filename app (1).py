...
class EfficientNetCBAM(nn.Module):
    def __init__(self, num_classes=23):
        ...
        self.cbams = nn.ModuleList([
            CBAM(88), CBAM(120), CBAM(208), CBAM(352)
        ])
...
st.title("🩺 Ứng dụng Chẩn đoán Bệnh Da Liễu")
...
class_names = [
    ...
    "Psoriasis pictures Lichen Planus and related diseases", "Rosacea", "Seborrheic Keratoses and other Benign Tumors",
...
# Upload ảnh
uploaded_file = st.file_uploader("📤 Chọn ảnh da cần chẩn đoán", type=["jpg", "jpeg", "png"])
...
if uploaded_file is not None:
    ...
    for i in range(3):
        ...
        st.metric(
            label=f"Top {i+1}: **{class_names[top_idx[i]]}**",
            value=f"{confidence:.2f}%"
        )

    # Nút tải mô hình
    with open("efficientnet_b2_cbam_best.pth", "rb") as f:
        st.download_button(
            label="📥 Tải mô hình (.pth)",
            data=f,
            file_name="efficientnet_b2_cbam_best.pth",
            mime="application/octet-stream"
        )

else:
    st.info("👆 Vui lòng upload ảnh da từ sidebar để bắt đầu dự đoán.")
